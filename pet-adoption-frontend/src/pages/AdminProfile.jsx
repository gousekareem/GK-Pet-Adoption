...
function AdminProfile() {
  ...
  return (
    <div className="admin-profile-container">
      <h1 className="admin-title">Admin Directory</h1>
      <p className="admin-subtitle">Welcome! Your admin control panel</p>
      ...
      {showAdmin && (
        <div className="admin-card">
          <h2>Admin Details</h2>
          <p><strong>Your cute name:</strong> {admin?.name || 'N/A'}</p>
          <p><strong>Your Email:</strong> {admin?.email || 'N/A'}</p>
        </div>
      )}

      {showUsers && (
        <div className="user-logins">
          <h2>User Registred Panel</h2>
          {loadingUsers && <p>Users on the way</p>}
              ...
              <div key={index} className="user-card">
                <div className="user-profile">
                  {user.profilePhoto ? (
                    ...
                    <div className="user-avatar-placeholder">DP your Photo please</div>
                  )}
                </div>
                <div className="user-details">
                  <p><strong>Your cute Name:</strong> {user.username}</p>
                  <p><strong>Your Email:</strong> {user.email}</p>
                  <p><strong>Your Status:</strong> {user.status}</p>
                  <p><strong>Your Zone:</strong> {user.zone}</p>
                  <p><strong>Cute Pets Donated:</strong> {user.petCount || 0}</p>
                </div>
